...
def _source_registry() -> list[list[str]]:
    return [
        ...
        ["Клиентский KPI OSA/TOP16", "data/raw/kpi/fact kpi/*.xlsx", "ТТ за месяц", "назначенный KPI, план и факт OSA/TOP16", "kpi_client_tt_long; kpi_client_tt_fact"],
        ["Клиентская SQL", "data/raw/kpi/sql_exports/HN_KPI_YYYYMM.zip", "подтверждённый визит", "дата, ID визита, ТТ, агент, сеть, PICOS потенциал/план/факт", "rtm_employee_visits; PICOS по визиту"],
...
def _etl_steps() -> list[list[str]]:
    labels = [
        ...
        ("KPI и визиты", "client KPI + клиентская SQL + ТТ→ТМ + USERS", "KPI-факты и аудиты"),
...
def _add_common_rules(parts: list[str]) -> None:
    ...
    parts.extend([
        ...
        _bullet("PICOS, OSA и TOP16 валидны только при наличии назначенного KPI, положительного плана и факта."),
        _bullet("Выполнение каждого KPI равно факт / план: ниже 75% результат обнуляется, выше 100% ограничивается значением 100%."),
        _bullet("Excel определяет тип KPI точки; PICOS план и факт берутся только из SQL-визита, OSA и TOP16 — из Excel."),
        ...
        _bullet("KPI сотрудника считается по его подтверждённым SQL-визитам."),
    ...
    parts.append(_table([
        ["Шлюз визита/KPI", "Да", "Нет"],
        ["Есть дата, ID визита, выполнен и подтверждён", "продолжить", "удалить визит и записать аудит"],
        ["Агент SQL однозначно связан с сотрудником", "продолжить", "сотрудник не назначается"],
...
def _add_kpi_methodology(parts: list[str]) -> None:
    ...
    parts.extend([
        _bullet("Тип KPI точки определяется по клиентскому Excel-файлу."),
        _bullet("PICOS: план % = плановый балл / потенциальный балл; факт % = фактический балл / потенциальный балл. Источник — конкретный SQL-визит."),
        _bullet("OSA и TOP16: план и факт берутся из соответствующих полей клиентского Excel-файла."),
        _bullet("Выполнение KPI = факт / план. Если выполнение ниже 75%, результат равен 0%; если выше 100%, результат равен 100%."),
        _bullet("Пустой план или факт остаётся пустым и не заменяется другим KPI."),
        ...
        _bullet("KPI сотрудника = среднее результата по всем его подтверждённым визитам с валидным KPI."),
...
def _add_limitations(parts: list[str]) -> None:
    ...
    parts.extend([
        ...
        _bullet("USERS определяет актуальных людей; клиентская SQL сохраняет факт визита и PICOS по визиту."),
...
def build_document() -> Path:
    ...
    parts.append(_table([
        ...
        ["client KPI + клиентская SQL + ТТ→ТМ", "KPI-схема и подтверждённые визиты", "Page1–Page5, Page7, Page8"],
